...
2)方式一:直接暴力破解，定0.5s的时间，就一直进行next去看是不是变成null了，如果变成null，代表这个莲表示单链表，否则就是cycle.
```python
import time
...
## 结束语
本文从数组与链表的基础讲起，到作LeetCode习题，不仅仅是学习两个数据结构，做两道题，而是以后遇到就不在惧怕的过程。

做算法题，确实烧脑，这不是一般的烧脑。希望自己一个月之内刷个三五百道。
